...
function nameCheck(name) {
    ...
    switch (true){
        ...
        case name.includes('Steven'):
            return ("What is up Steven?");
            break;

        case name.includes('Bryan'):
            return ("Hey Bryan!");
            break;
...
for (j=0; j<arrayEvaluator.length; j++) {
    console.log(arrayEvaluator[j]);
}
...
function theEliminator(contestants, loser) {
    ...
    while (!found && i<contestants.length) {
...
let remaining = theEliminator(contestants, loser);
...
let up = printUpper(sampleString);
console.log(up);
...
let checkResult = emailCheck('     robsim37@gmail.com      ');
...
//CODE HERE
function frogBuyer (galleons) {
    return Math.floor(galleons/3); //Honestly, I saw this issue prior to being propted in the next question...
}

let totalFrogs = frogBuyer(31);
...
//CODE HERE
function frogBuyer (galleons) {
    return Math.floor(galleons/3);
}

let totalFrogs2 = frogBuyer(72);
...
console.log(isAscending([0,1,2,3,4,5,6,8,9]));
